...
class EditProductDetails extends StatefulWidget {
  ...
  const EditProductDetails({this.proData});
...
//------------------------- Load item type dropdown data -------------------------
  ...
  void loadAllData() {
    selectedItemType = "${widget.proData['type']}" == "veg"
        ? "Vegetable"
        : "${widget.proData['type']}" == "fru"
            ? "Fruit"
            : "Grain";
    selectedTypeForFirebase = "${widget.proData['type']}";

    selectedItemName = "${widget.proData['product_name']}";
    minimumSupply = "${widget.proData['min_supply']}";
    maximumSupply = "${widget.proData['max_supply']}";
    pricePerKilo = "${widget.proData['price']}";
    itemLocation = "${widget.proData['location']}";
    itemDescription = "${widget.proData['description']}";
    image1Url = "${widget.proData['photo_url_1']}";
    image2Url = "${widget.proData['photo_url_2']}";
    image3Url = "${widget.proData['photo_url_3']}";
    image4Url = "${widget.proData['photo_url_4']}";
    documentId = widget.proData.id;

    _minimumSupply.text = minimumSupply;
    _miaximumSupply.text = maximumSupply;
    _pricePerKilo.text = pricePerKilo;
    _itemLocation.text = itemLocation;
    _itemDescription.text = itemDescription;

    users.doc(userIdGlobal).snapshots().listen((event) {
      sellerName = event.data()['first_name'] + ' ' + event.data()['last_name'];
...
//-------------------------------------------------------------------------------
  Future updateSalesToFirebase() async {
    CollectionReference sales = FirebaseFirestore.instance.collection('sales');
    final userId = userIdGlobal;
    final currentDateTime = DateTime.now();
